...
if (!fs.existsSync(tfFilePath)) {
  throw new Error(
    'Terraform state file does not exist! Have you run "terraform apply"?'
  );
}
...
(async () => {
  console.log("Deploying in 3 seconds... BE PREPARED!");
  await new Promise(resolve => setTimeout(resolve, 3000));

  const lockFilePath = rel("../deploy.lock");
  console.log("Checking for lockfile...");
  if (fs.existsSync(lockFilePath)) {
    console.error("Lockfile dploy.lock found! Wooohooo! lets Halt...");
    process.exit();
  }

  console.log("Creating lockfile... stay intact!");
  fs.writeFileSync(
    lockFilePath,
    "This stops node-deploy from running concurrently with itself. Remove this if node-deploy complains."
  );

  console.log("Checking environment... something you're hiding o.O?");
  if (!fs.existsSync(rel(".production.env"))) {
    console.error("No .production.env found! Halting...");
    process.exit();
  }

  console.log("Copying appspec... I hope you have SSD or better M.2 U.U");
  fs.copyFileSync(rel("./appspec.yml"), rel("../appspec.yml"));

  console.log("Generationg deployment file... put on your seat belt!");
  const filename = `${deploymentDirName}-deployment-${getFullDate()}.zip`;
  const zipPath = `/tmp/${filename}`;
  await exec(
    `zip -r ${zipPath} . -x terraform/\\* -x node_modules/\\* -x \\*/node_modules/\\* -x \\*/.cach/\\* -x .git/\\* -x \\*.DS_Store`,
  ...
  console.log("Deploying applicationCache...");
